...
class URConnection(object):
    ...
    def connect(self):
        ...
        self.__thread = threading.Thread(
            name="URConnection", target=self.__run)
    ...
    def __on_packet(self, buf):
        ...
        # Updates the state machine that determines whether we can program the
        # robot.
        can_execute = (
            state.robot_mode_data.robot_mode in [RobotMode.READY, RobotMode.RUNNING])
                    ...
                    while len(self.__buf) >= 48:
                        ...
                        if len(self.__buf) >= packet_length:
                            packet, self.__buf = self.__buf[
                                :packet_length], self.__buf[packet_length:]
...
class URConnectionRT(object):
    ...
    def connect(self):
        ...
        self.__keep_running = True
        self.__thread = threading.Thread(
            name="URConnectionRT", target=self.__run)
        self.__thread.daemon = True
        self.__thread.start()
...
class TCPServer(SocketServer.TCPServer):
    # Allows the program to restart gracefully on crash
...
def reorder_traj_joints(traj, joint_names):
    ...
    for p in traj.points:
        new_points.append(JointTrajectoryPoint(
            ...
            velocities=[p.velocities[i]
                        for i in order] if p.velocities else [],
            accelerations=[p.accelerations[i]
                           for i in order] if p.accelerations else [],
...
def interp_cubic(p0, p1, t_abs):
    ...
    for i in range(len(p0.positions)):
        a = p0.positions[i]
        b = p0.velocities[i]
        c = (-3 * p0.positions[i] + 3 * p1.positions[i] - 2 *
             T * p0.velocities[i] - T * p1.velocities[i]) / T ** 2
        d = (2 * p0.positions[i] - 2 * p1.positions[i] +
             T * p0.velocities[i] + T * p1.velocities[i]) / T ** 3
...
class URTrajectoryFollower(object):
    ...
    def __init__(self, robot, goal_time_tolerance=None):
        ...
        self.robot = robot
        global prefix
        self.server = actionlib.ActionServer(prefix + "follow_joint_trajectory",
    ...
    def on_cancel(self, goal_handle):
        ...
        if goal_handle == self.goal_handle:
            with self.following_lock:
                ...
                point0.time_from_start = rospy.Duration(0.0)
                point1 = sample_traj(
                    self.traj, now - self.traj_t0 + STOP_DURATION)
            ...
            if (now - self.traj_t0) <= self.traj.points[-1].time_from_start.to_sec():
                ...
                try:
                    self.robot.send_servoj(
                        999, setpoint.positions, 4 * self.RATE)
                except socket.error:
                    pass
                ...
                position_in_tol = within_tolerance(
                    state.position, last_point.positions, self.joint_goal_tolerances)
                ...
                if self.goal_handle:
                    ...
                    if position_in_tol and velocity_in_tol:
                        # The arm reached the goal (and isn't moving).
                        # Succeeding
